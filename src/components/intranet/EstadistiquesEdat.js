...
export default function EstadistiquesEdat({ castellers = [] }) {
  ...
  const estadistiques = useMemo(() => {
    ...
    castellers.forEach((casteller) => {
      // Només comptem els castellers actius
      if (casteller.actiu !== false) {
        ...
        const neixement = new Date(casteller.dataNaixement);
        let edat = anyActual - neixement.getFullYear();
        const mes = new Date().getMonth() - neixement.getMonth();
        
        if (mes < 0 || (mes === 0 && new Date().getDate() < neixement.getDate())) {
          edat--;
        }

        // Clasificació per franges d'edat requerides
        if (edat >= 0 && edat <= 15) {
